...
/**
 * Interactive CLI tool for pushing dummy mobile operator call records (CDRs) to Kafka.
 *
 * Run via:  mvn exec:java -Dexec.mainClass="com.snowflake.streaming.producer.FakeKafkaWriter"
 */
public class FakeKafkaWriter {

    private static final Logger logger = LoggerFactory.getLogger(FakeKafkaWriter.class);
    ...
    public FakeKafkaWriter(Config config) {
        ...
        this.producer = new KafkaProducer<>(config.buildKafkaProducerProperties());
    }

    public static void main(String[] args) throws Exception {
        String configPath = System.getProperty("config.path", "producer-config.properties");
        Config config = new Config(configPath);
        new FakeKafkaWriter(config).runInteractive();
    }
    ...
    private void printMenu() {
        System.out.println();
        System.out.println("=== Fake Kafka Writer — Mobile CDR Generator ===");
    ...
    private void sendSingle() throws Exception {
        ...
        String callerId = (String) cdr.get("caller_number");
        send(callerId, mapper.writeValueAsString(cdr));
    ...
    private void sendBurst(int count) throws Exception {
        ...
        for (int i = 0; i < count; i++) {
            ...
            Map<String, Object> cdr = generateCDR(seq);
            String callerId = (String) cdr.get("caller_number");
            producer.send(new ProducerRecord<>(topic, callerId, mapper.writeValueAsString(cdr)));
        }
        producer.flush();
        ...
        Thread streamThread = new Thread(() -> {
            try {
                while (streaming.get()) {
                    ...
                    Map<String, Object> cdr = generateCDR(seq);
                    String callerId = (String) cdr.get("caller_number");
                    producer.send(new ProducerRecord<>(topic, callerId,
                            mapper.writeValueAsString(cdr)));
    ...
    private void sendMalformedMessages() throws Exception {
        send("malformed-1", "not json at all");
        send("malformed-2", "{broken json: ");
        send("malformed-3", "");
        send("malformed-4", "null");
        send("malformed-5", "[1, 2, 3]");
        producer.flush();
    ...
    private void sendNullFields() throws Exception {
        ...
        send("nulls-1", mapper.writeValueAsString(missingCallee));
        ...
        send("nulls-2", mapper.writeValueAsString(nullDuration));
        ...
        send("nulls-3", mapper.writeValueAsString(allNulls));

        Map<String, Object> empty = new LinkedHashMap<>();
        send("nulls-4", mapper.writeValueAsString(empty));
    ...
    private void sendCustomMessage(String json, Scanner scanner) throws Exception {
        ...
        send("custom-" + sequence.incrementAndGet(), json);
    ...
    private void send(String key, String value) throws Exception {
        ...
        logger.info("Sent -> partition={}, offset={}, key={}, size={}B",
                meta.partition(), meta.offset(), key, value.length());
    }

...
Example repository demonstrating the **Snowpipe Streaming SDK** (v2) for real-time data ingestion into Snowflake. Contains minimal SDK examples (Java, Python) and advanced integration examples (Kafka, Pub/Sub) with real-world patterns.
...
custom-pubsub-consumer/    # Pub/Sub → Snowflake consumer (Java 17+, SDK 1.3.0)
  pom.xml
  consumer-config.properties
  producer-config.properties
  profile.json.example
  src/main/java/com/snowflake/streaming/
    consumer/               # Main, CustomPubSubConsumer, Config
    producer/               # FakePubSubWriter (CDR generator), Config
...
- **CDR data model**: Both Kafka and Pub/Sub examples use the same 15-column call detail record schema for consistency.
...
**Pub/Sub consumer**: `cd custom-pubsub-consumer && mvn compile exec:java -Dexec.mainClass="com.snowflake.streaming.consumer.Main"`
**Pub/Sub producer**: `cd custom-pubsub-consumer && mvn compile exec:java -Dexec.mainClass="com.snowflake.streaming.producer.FakePubSubWriter"`
